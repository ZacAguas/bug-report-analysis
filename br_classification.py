...
pd_tplusb = pd_all.rename(
    columns={"Unnamed: 0": "id", "class": "sentiment", "Title+Body": "text"}
)
pd_tplusb.to_csv(
    "Title+Body.csv", index=False, columns=["id", "Number", "sentiment", "text"]
)
...
# Save scores to results csv
import json

with open(f"results/{project}_scores.json", "w") as f:
    json.dump(
        {
            "f1": f1_scores,
